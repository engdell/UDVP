...
-- Stored procedure consolidated for the task
CREATE OR REPLACE PROCEDURE RUN_DOCUMENT_PROCESSING()
RETURNS STRING
LANGUAGE SQL
EXECUTE AS OWNER
AS
...
BEGIN
    ...
    -- Attempt to refresh the Cortex Search Service after documents finish processing
    BEGIN
        ALTER CORTEX SEARCH SERVICE DOCUMENT_SEARCH_SERVICE REFRESH;
    EXCEPTION
        WHEN STATEMENT_ERROR THEN
            -- Service may not exist yet (e.g., during initial setup); swallow error
            NULL;
    END;

    RETURN 'PROCESS_DOCUMENTS_TASK completed';
END;
$$;
